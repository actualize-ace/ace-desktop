...
|---------|--------|----------|------|
| Auto-reload UX — warn-before-reload | Done | High | refresh-engine's full-reload was silent and dropped in-memory chat history. Bumped `FULL_RELOAD_IDLE` 6h → 12h and added a 30s warning toast with Postpone button before `location.reload()` fires. [refresh-engine.js:97-176](renderer/modules/refresh-engine.js#L97-L176). |
| User comms — chat history recovery | Not started | High | Tell Mac users they can resume any prior chat via `claude --resume <session-id>` in Terminal (and via the in-app History view). One-time email + add to onboarding doc + add a hint in the post-reload restore toast. Blocked-on-Windows: `claude --resume` flow doesn't work on Windows yet — needs investigation before we send blanket comms. |
| Windows chat history recovery | Not started | High | `claude --resume` from Terminal (the Mac fallback when in-app history fails) doesn't work on Windows. Investigate: is it the CLI itself, the session-id storage path, or the in-app History view's resume button? Until this works, Windows clients have no recovery path if the auto-reload (or any crash) fires mid-session. Pair with Phase 2 Windows port. |
| ~~Electron upgrade (28 → 34)~~ | Done | ~~High~~ | Shipped pre-client deploy. Electron 34.5.8, Node 20 LTS, Chrome 132. Commit `a67daa7`. |
| lean-ctx cherry-picks (3 tracks) | Not started | High | **Track A (30 min):** Install lean-ctx binary + wire Claude Code PreToolUse hook for shell/file compression in vault sessions. **Track B (15 min):** Add read mode constraints (signatures/map/full) to `build-mode` SKILL.md. **Track C (2–3h):** Mid-session finding capture in ACE Desktop — pin button on tool cards → `session-findings.json` → auto-surfaced at `/close`. Addresses lost-session data loss. Source: [github.com/yvgude/lean-ctx](https://github.com/yvgude/lean-ctx). [Plan](docs/plans/2026-04-21-lean-ctx-cherry-picks.md) |
...
| Skill system quality uplift → v0.3.0 | Not started | Medium | Adopt 4 patterns from msitarzewski/agency-agents into ACE's `.claude/skills/` system: (1) skill spec + template at `00-System/specs/skill-spec.md` with required frontmatter (`voice`, `success_signal`, `tools`) and required body sections (Purpose, Reads, Writes, Constraints, Workflow, Failure Modes); (2) `voice` field — one-line tone/register that solves recurring drift (ghostwrite voice, list-email register, email AI tells); (3) `success_signal` field — observable success state per skill, makes regressions like Marc's `/start` visible faster; (4) `tools/lint-skills.sh` — bash linter wired to pre-commit, warn-only for 30 days then blocks. Migration is opt-in via natural touch — no sprint. First 5 skills: `/start`, `/close`, `/coach`, `/capture`, `/ghostwrite`. Source: [docs/plans/2026-04-25-skill-system-quality-uplift-design.md](../docs/plans/2026-04-25-skill-system-quality-uplift-design.md). |
| Widget startup race — DCA + command bar empty on launch → v0.2.3 | Not started | High | Diagnosed 2026-04-22 on Eliana live call. Widgets call `render()` before composite data resolves → hit empty-state branch and never re-render. `location.reload()` reliably fixes. Intermittent by machine (disk I/O speed decides the race). Fix: re-render trigger on data arrival — either `dash.onDashRefresh` listener in [dashboard.js](renderer/dashboard.js) (preload bridge already exposes it) or `Promise.all` gate so widgets don't mount until all `getNorthStar` / `getCommands` / composite sources resolve. One commit, covers North Star + command bar. Scope: ~1h. |
| Seed `.claude/settings.json` in `/build-vault` → v0.2.3 | Not started | High | **Why clients hit WebFetch + git dead-ends:** build-vault policy at [SKILL.md:618](/.claude/skills/build-vault/SKILL.md#L618) says *"No `.claude/settings.json` customization. Clean template. Permissions accumulate during use."* That assumption depended on the in-chat permission prompt UI working, but that UI is currently broken ([backlog permission UI bug](Domains/actualize/backlog.md)) — so nothing accumulates. Every new client blocks on first WebFetch, first git commit, first git push. Hit twice in 24h (Eliana + Patrick, 2026-04-22). **Fix:** change build-vault to seed a baseline `.claude/settings.json` (tracked, committed — ships with the repo so clients get it on clone) in every new client vault, covering: `WebFetch`, `Bash(git add:*)`, `Bash(git commit:*)`, `Bash(git push:*)`, `Bash(git pull:*)`, `Bash(git fetch:*)`, `Bash(git status)`, `Bash(git diff:*)`, `Bash(git log:*)`, `Bash(git checkout:*)`, `Bash(git branch:*)`, `Bash(gh *)`. Also add `.claude/settings.local.json` to `.gitignore` so clients can layer personal per-machine overrides without leaking into the shared repo. Update the "clean template" rhetoric to "seeded safe baseline (tracked) + personal overrides (ignored); operator tightens per-client if needed." Also retrofit all existing client vaults (Eliana, Patrick, Craig, Marc, Joe, Aleks, Kim) so they don't hit this again. Scope: ~1h skill edit + 15min × 7 clients = ~3h total. Underlying permission-UI bug remains a separate fix. |
| Coherence Orb stale after /pulse | Not started | High | **Bug:** Orb reads `metrics._signals` loaded from `system-metrics.md` once at `loadDashboard()`. Running `/pulse` writes a new snapshot but no `cockpit-refresh` event fires — dashboard never re-reads the file. Orb shows prior session's score until app restarts or user clicks a triad-leg card button. **Root cause:** `cockpit-refresh` dispatch is wired only inside `triad-leg.js` (6 click handlers); chat commands have no path to trigger it. **Fix (two parts):** (1) Expose `window.__refreshCockpit = () => window.dispatchEvent(new Event('cockpit-refresh'))` in `dashboard.js`. (2) In the preload bridge, add an IPC listener (`ipcRenderer.on('vault-metrics-updated', ...)`) that calls `window.__refreshCockpit()`. Then have `vault-reader.js` emit `vault-metrics-updated` to the active window after any `GET_METRICS` write completes — or, simpler, have the pulse skill send a one-line IPC ping via the terminal's `shell.send`. Scope: ~1–2h. |
| Post-wake renderer starvation handling | Not started | High | **Bug:** When the app is left open overnight and Mac wakes from sleep, the renderer gets main-thread starved for ~3 minutes as macOS re-schedules many apps simultaneously. Symptoms: UI feels frozen / unresponsive, heartbeat overruns at ~900ms every 1s in `~/Library/Logs/ACE/longtask.log`, but zero `performance-observer` longtask entries (no single blocking task — it's OS-level starvation). Observed 2026-04-21 morning on Nikhil's machine (uptimeMs 41min of `performance.now()` time but wall-clock was overnight). Heap 11MB, DOM 1648 nodes — no accumulation, not a code-level jam. **Fix (three parts, any one or combination):** (1) Main process listens to `powerMonitor.on('resume')` and broadcasts a `system-wake` IPC to renderer. Renderer triggers soft GC + shows a brief "Resuming…" status dot for 2–3s. (2) Detect heartbeat-overrun cluster in longtask-observer.js itself — if ≥10 overruns in 10s, surface a non-modal nudge ("System resuming — tap to refresh") so user has agency instead of staring at a frozen-looking UI. (3) Drop `MIN_UPTIME` in refresh-engine.js from 2h to 30min so long-idle instances are more aggressive about full reload on resume. **Not blocked by current perf-hardening sprint** (Phase A/B/C target different failure modes). Scope: ~2–3h once Phase B validated with Aleks. |
| History → chat-view rehydration | Not started | Medium | **Bug:** Opening a session from History view lands in terminal (xterm) mode. Clicking the "Chat" mode toggle on the header doesn't switch — session stays wedged in terminal. Expected: history entries open in chat view by default, or the toggle actually swaps views on a history-loaded session. **Hypothesis:** chat-view is populated from `session.messages` at render time; history-opened sessions hydrate the xterm transcript but never repopulate `#chat-msgs-${id}`, so flipping the toggle reveals an empty chat-view and bounces back. **Fix (sketch):** on mode toggle, if target is chat-view and `msgsEl` has no `.chat-msg` children, rebuild from `session.messages` before showing. Observed 2026-04-21 during Phase A baseline prep. Scope: ~1–2h. |
| Live health & performance panel | Not started | Medium | **Value:** Clients self-report specific numbers ("15 FPS, 380MB heap after 3 hours") instead of vibes ("feels slow"). Nikhil spots drift proactively. Bug reports become actionable instantly. **Anchor:** existing vitals dot (from refresh-engine). Click → expandable mini-panel docked next to it. **Signals already collected (free to render):** health score 0–1 + per-sensor breakdown (`_refreshEngine.sensors()`), jsHeapMB, DOM node count, active chat count, visible view, overrun stack traces (from `longtask-observer.js`), next soft-GC ETA, uptime since last full reload. **Signals to add (cheap):** FPS via rolling 60-frame rAF avg (~1% CPU), last-60s overrun count (aggregate what's already being logged), IPC event rate (already visible in chat-manager batching), per-process CPU/RAM via `app.getAppMetrics()` pushed from main every 5s. **Three ship options, pick by scope:** (1) Hover-expand on vitals dot — mini panel with FPS · Heap · DOM · Overruns-last-60s · Uptime · Next GC. ~1 day. (2) Ambient bottom-right status bar always visible, 4 numbers, click expands to #1. ~1.5 days. (3) Full dev panel behind Cmd+Shift+H with live graphs over time + export-to-JSON for remote debugging. ~2–3 days. **Recommended start:** option 1, extend to 3 when remote-debugging a client becomes a recurring pattern. **Don't-monitor-the-monitor caveat:** FPS loop pauses when panel is closed; updates throttled to 1Hz; panel opt-in (closed by default). Captured 2026-04-21 while diagnosing Aleks's freeze — would have made yesterday's investigation 10× faster. |
...
| Local ACE health agent | Background cron agent monitors vault + system health (staleness, integrity signals) and pushes summaries via MCP or GitHub notifications. No auth surface required — reads existing health signals. Pairs with client health dashboard (Client Infrastructure section). Captured 2026-04-22. | — |
