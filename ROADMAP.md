...
| File attachment in chat (PDF, docs) | Not started | Medium | File picker in chat input → local path passed to Claude CLI. Filename chip in composer. Unblocks Craig's PDF workflow. v0.2.x. |
| Image paste in chat (screenshots) | Not started | Medium | Intercept clipboard paste, detect image, write to temp file via IPC, inject path into message. Thumbnail preview in composer. Craig requested. v0.2.x. Dependency: verify Claude CLI stream-json mode handles file args same as PTY. |
