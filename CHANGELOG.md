...
## [v0.4.8-rc.2](https://github.com/actualize-ace/ace-desktop/releases/tag/ace-desktop-v0.4.8-rc.2) · 2026-09-24 · the Core update pill reaches the vaults furthest behind, and two cards go quiet

Release candidate 2 toward 0.4.8. Three changes, all found installing rc.1 for a member.

- **A vault with no record of a Core release now gets the update pill.** The pill measured how far behind a vault is from `.ace/core-version.txt` or `.ace/core-reviewed.txt`, and said nothing when neither existed, to avoid guessing. That hid it from exactly the vaults furthest behind: a member whose vault was built in June and never synced since saw no pill on rc.1, and found out only from the memory card. It was not a guess to be avoided: `/sync-core` has run the upstream engine since Core v1.8.2, and every engine from v1.19.7 writes `core-reviewed.txt` whenever a sync ends with every item decided, holds included. So a vault linked to Core with neither file has never finished a sync on v1.19.7 or later and is behind. It now shows "Core update · <latest>" with no count, since how far behind cannot be known, and the first finished sync writes the record and darkens it. A vault not linked to Core, a remote named `upstream` that does not carry Core's sync engine (a template or fork, which no sync could ever darken), or a Core whose newest release predates v1.19.7 still shows nothing. A vault cloned onto a second machine before stamps were committed shows the pill until its first `/sync-core` check, which clears it. (`src/core-update.js`, `renderer/modules/core-update-pill.js`)

- **The memory card no longer repeats the Core update pill.** A member months behind met a paragraph-long "ACE is running an older version of its memory" card at the top of every new chat, every launch, whose one button ran `/sync-core`, the same thing the titlebar pill asks for in one line. A memory card whose fix is an update now hands its Core answer to the pill and steps aside only when the pill is carrying it: on screen, or already set aside by the member (× or the snooze after opening a sync, which the card does not overrule). Otherwise the card appears as before, because a vault can be on the latest Core release and still behind on memory (a member keeping their own memory skill), and a launch-time pill answer taken offline can be stale: said once, never zero times. The card also no longer lands in a chat the member has already started, and two chats opened during the check raise one card, not two. The pill and the card share one Core check for a minute, invalidated the moment a sync writes its record, instead of running two network fetches against the same repository at once. (`renderer/modules/memory-status-card.js`, `renderer/modules/core-update-pill.js`, `main.js`)
- **The Guide's first-run card is gone.** "The Guide lives here now" described how the Guide was built ("perception runs locally", "never a queue") rather than what it does, and it read as noise to a member meeting it for the first time. The orb's hover text names it, and right-click still carries quiet mode, the focus timer and the off switch. (`renderer/modules/guide-hearth.js`, `renderer/styles/guide.css`)

**Verified on this candidate:** 14 new tests and 1 changed (4 on real git repositories for the Core check, 7 for the memory card's yield rule and raise path, 3 for the pill; the Guide test now asserts the card never mounts), each mutation-tested red (12 mutations, all caught; the Guide test also fails against the 0.4.8-rc.1 code); a local code review (0 critical, 4 important, 4 minor), every important and three minors fixed and re-verified; the Core check run against the operator's real ACE Core repository from a throwaway vault with no record (pill shown, `v1.21.3`), then with a full-sync record (dark), then four releases behind (count 4). **Not yet verified:** the pill and the quieter memory card seen on a member machine.

## [v0.4.8-rc.1](https://github.com/actualize-ace/ace-desktop/releases/tag/ace-desktop-v0.4.8-rc.1) · 2026-09-24 · ACE keeps Claude Code new enough for the models it offers

Release candidate 1 toward 0.4.8. One change, prompted the day 0.4.7 shipped.

- **ACE updates Claude Code by itself when a model needs a newer one.** Opus 5.5 needs Claude Code 2.1.280 and Fable 5.1 needs 2.1.251. A member on 2.1.206 picked Opus 5.5 and every turn ended in a raw HTTP 400 telling them to run a terminal command (reported 2026-09-24 by a member with a screenshot). Five headless runs of 2.1.206 never moved its version, so a member who only meets Claude Code through ACE can sit months behind; the same drift was diagnosed on the operator machine in May (`docs/plans/2026-05-28-claude-cli-autoupdate-health-plan.md`, cause there `DISABLE_AUTOUPDATER`), and the ACE-side updater that plan proposed was never built. Whether the member's machine also had `DISABLE_AUTOUPDATER` set is not known; if it did, this release leaves it alone and shows the switched-off card. Now, a few seconds after launch, ACE checks the version and, if it is below the newest floor, updates it in the background while a titlebar pill says so, then says it is done and leaves. Nothing shows when Claude Code is already current. (`src/claude-runtime/self-update.js`, `renderer/modules/claude-update-pill.js`, `main.js`)
- **The update follows rules found by running the real CLI, on macOS locally and on GitHub's Windows and Linux runners** (`.github/workflows/claude-update-windows.yml`, branch `test/claude-update`): it runs with ACE's full PATH, because an app's thin PATH cannot find npm; it aims an npm update at the install's own prefix, because otherwise the CLI updates a different copy while printing "Successfully updated" (on Windows the original then stopped launching); it counts success only from the version the binary reports afterwards; when the install folder cannot be written it sets up a native copy in the member's home with no password (`claude install`, proven on all three platforms) and switches ACE to it; it waits until no chat is running, because Windows refuses to replace a binary in use, and gives up after ten minutes with a message rather than waiting forever; a chat sent while the binary is actually being replaced waits a moment instead of launching a half-written install; a copy an update left unable to start is reinstalled in its own prefix; it never updates a Homebrew copy behind brew; and it leaves Claude Code alone when the member set `DISABLE_AUTOUPDATER`. It deliberately ignores `autoUpdates: false` in `~/.claude.json`, which the CLI writes itself.
- **"This model needs a newer Claude Code" gets its own card.** The too-old refusal no longer lands on the generic card with Retry and Reset Context, neither of which can update anything. The new card offers **Update and retry**, updates in place, and resends the message. It asks for the version the refused model named, which can be newer than ACE's own floor. If the update cannot run, it says why in plain words and offers **Fix with ACE** (a new chat on Opus 4.8, which the old Claude Code still runs), **Copy details**, and **Try again**. No card ever names a terminal command or sudo. (`renderer/modules/claude-update-copy.js`, `renderer/modules/session-manager.js`)

**Verified on this candidate:** 51 new unit tests plus 1 changed call-site test, each safety rule mutation-tested red (19 mutations over two rounds, all caught); a local code review (1 critical, 6 important, 11 minor), with the critical, every important and the minors that change behaviour fixed and re-verified; ACE's own update code run against Claude Code 2.1.206 with an app-thin PATH (updated to 2.1.281) and against an unwritable install (switched to a native 2.1.282, old copy untouched); the dev app launched with 2.1.206 and updated it on its own four seconds after start (`[claude-update] updated 2.1.206 -> 2.1.281`); on a machine with `DISABLE_AUTOUPDATER=1` the same launch left it alone. All re-run after the review fixes. Full suite 6098 passed, 1 failed, 5 skipped; the one failure is the main-thread timing test in `history-render.test.js`, which passes in isolation, and every other failure seen across earlier runs was likewise a timing test that passed alone on a machine running at load average 17 to 43. **Not yet verified:** the pill and the new error card seen on screen, the error card's Update and retry path in a live chat, Homebrew installs, and Windows folders that need admin rights.
...
- **The pane reloads on a change to the disk, not only on a change ACE made.** The open pane's only "the page changed" signal was the tool seam: a completed Write, Edit or NotebookEdit. A shell edit from Bash, a build script regenerating a site, a save from another editor, or a stylesheet beside the entry changed the bytes and the pane went on showing the old render with nothing on screen saying it was stale. Observed 2026-09-21: three shell edits to an open test page, the pane frozen on the first render. While a pane is open, ACE now watches that page's folder on disk and feeds any change into the same debounced path the tool seam uses, so the two signals coalesce and ACE's own edits do not reload twice. The watcher waits for a write to settle before it fires, which was the plan's original reason for having no watcher; it ignores dotfiles, editor backups and `node_modules`; it always names the entry page to the renderer, never the stylesheet, so no chip is ever offered to a `.css` file; and it lives exactly as long as the pane, closing on pane close, on renderer reload, and on its own when the page vanishes. Proven in the dev build: a `sed -i` on the heading colour of an open page reloaded the pane with no click. (`src/preview/preview-watch.js`, `main.js`)

- **A signed-out Claude account is named on screen, with a way back in.** A sign-in that expires while a turn is in flight reaches ACE on the result event, not on stderr where it was already caught, and fell through to the generic engine-error card whose two actions, Retry and Reset Context, cannot re-authenticate anything. Reported by a member with a screenshot of exactly that card. The card now says what happened and that nothing is lost, offers the in-app sign-in, and reveals the manual route only when the button cannot do the job. Also fixes the deprecated gold colour aliases resolving to the dark value in light mode. (`renderer/modules/account-auth-error.js`, `renderer/modules/session-manager.js`)

## [v0.4.7-rc.6](https://github.com/actualize-ace/ace-desktop/releases/tag/ace-desktop-v0.4.7-rc.6) — 2026-09-15 — the pane is the default surface, and ACE can open it

Release candidate 6, built locally through a scratch Node 20.20.2 (the system 20.12.2 sits under electron-builder's floor). rc.3 to rc.5 never got sections here; this candidate is the first installed build that carries the preview link gaps (a `target="_blank"` click raises the browser offer, a CSP-blocked navigation re-serves the page, the browser button follows the frame), page ownership (a page belongs to the chat that worked on it; anything else is parked), and the voice-recording format fix, alongside the three changes below.

- **A large folder is a site by composition, not a junk drawer by count.** The preview root policy refused any folder with more than twelve entries, which called a twenty-page website with `img/`, `docs/`, `_headers` and `netlify.toml` a folder of unrelated files. A folder of twelve entries or fewer is still dedicated on size alone; above that line it is a site when its folders are fewer than its files and its web files are at least twice its other files. A container of artifact folders refuses as one, a folder of notes with a page in it still refuses, dotfiles are neutral and a symlink is a document whatever it is called. On this vault 305 of 391 tracked pages now serve, 86 refuse. (`src/preview/preview-root-policy.js`)
- **ACE opens the pane when you asked to see a page.** "Pull up aurora's website" used to end with ACE saying a chip should be there and you still clicking. ACE now ends such a reply with a hidden `<!-- ace:show <path> -->` note; the app resolves the path inside the vault, records the sighting for that chat, and opens the pane with no click when the root policy serves the page. A refused page is recorded and not opened, and no browser is ever launched. Nothing opens unbidden: the note is emitted only for a page you asked for, a stranded or stopped turn drops it, and a page ACE merely read or wrote still arrives as the chip. (`renderer/modules/preview-show.js`, `renderer/modules/chat-renderer.js`, `src/chat-manager.js`)
- **The page count in the chat header glows when the set changes.** The chip follows the newest tool card and is out of sight once you scroll; the header icon is the stable place, so it glows three times and settles when a page is sighted, never on a restore at boot and never under reduced effects. The folder label in the page list also keeps its digits at the head: `11-Artifacts/files/x` had rendered as `Artifacts/files/x-11` on every artifact row. (`renderer/modules/preview-pages.js`, `renderer/styles/preview.css`)

**Verified on this candidate:** the full unit suite (4,867 tests across 262 files, 4 skipped), the module-graph check, 33 named mutations each proven red, and all three changes driven on screen in the dev build over CDP: the aurora site served in-pane, a real chat turn opened the pane eight seconds after send, and the header glow ran its three passes and came off at 2.1s.

## [v0.4.7-rc.2](https://github.com/actualize-ace/ace-desktop/releases/tag/ace-desktop-v0.4.7-rc.2) — 2026-09-09 — the page opens beside the chat, and ACE can say what it is doing

Release candidate 2. rc.1 (2026-09-04) carried the four Whisper fixes and the system-prompt correction and never got its own section here, so everything since v0.4.6 is listed under this heading. This candidate is the first tagged build to carry the in-chat preview pane, the runtime signal and the Activity view; members have been running the vault rule that prefers the pane since 2026-09-08 without the pane itself, and this closes that gap.

**Verified on this candidate:** the full unit suite (4,200 tests across 238 files, 4 skipped), the module-graph check, a local arm64 DMG built from this tree, and a hands-on pass of the packaged app on macOS in a fresh profile: preview offer, open, select-to-edit, width clamp, pages list and reload; chat switching with an open pane; Activity *Working* and *Finished* for a real chat turn, named by the turn note and carrying its receipt; quit, relaunch and checkpoint recovery of three chats, the transcript and the finished result; and one scheduled task dispatched headless through the packaged binary to a completed daemon job with its health sidecar written. See the release record in `docs/plans/2026-09-09-v0.4.7-rc.2-release-record.md` for what each check actually observed.

**Not verified on this candidate:** *Blocked on you* (no permission park was triggered) and the *Earlier* fold (no run was older than the live view); Windows and Linux (no build, no run); the AVX2 Whisper compile flag (Apple Silicon cannot produce the x64 binary; it stays a CI-only proof); and the freeze/reply-loss cluster, which remains open and is why this is still an rc. One gap was observed and is recorded rather than fixed: the headless-dispatched job ran to done and showed its result in the Agents view, but no background-agent run reached the runtime journal, so Activity never listed it. Cause not established.

### Added
- **The page opens beside the chat.** When ACE writes, reads, runs a shell line against, or simply names an HTML page that lives in the vault, a small chip appears on that turn. Click it and the page opens in a pane beside the chat, at a draggable split, sandboxed and served from disk by ACE itself. It reloads on every completed write to the page it is showing, and only on a write: a Read of the open page never reloads it, and a sighting of a *different* page offers a chip to switch rather than reloading the wrong one. Switching to another chat folds the page away and the tab keeps a small mark; switching back brings it back without reloading. Closing the pane declines the pane, not the page, so the chip stays. Nothing opens unbidden: the chip is an offer, and the pane only ever appears on a click. Most vault HTML does not sit in a folder of its own and is refused by the preview's root policy; a refused page opens in the browser instead, and the chip carries the refusal's own sentence saying why. The pane is faithful to layout, not to third-party behaviour: remote fonts, scripts and fetches are blocked by policy and the bar says so. (`src/preview/`, `renderer/modules/preview-chip.js`, `renderer/modules/preview-pane.js`, `renderer/modules/preview-pages.js`, `renderer/styles/preview.css`, `docs/plans/2026-09-04-preview-iframe-spike-findings.md`, `docs/plans/2026-09-05-preview-steps-3-4-findings.md`)
- **Point at the page, and the page answers as an attachment.** The pane's bar carries a picker. Arm it, and a dashed outline follows the cursor over the page; click an element and it lands in the composer as a chip carrying its selector path and a snippet of its text, exactly like a file attachment. Picked elements take a solid outline, clicking one again removes it, and sending consumes the chips and clears the outlines. The picker stays armed after a capture, because pointing is usually plural. The page gets nothing it did not already have: the injected script only ever runs while the picker is armed, only in HTML, and only sets or clears attributes in the page's own document; every gate is on ACE's side of the channel, with a rate limit and a standing cap of twelve pending selections. Ten hostile message shapes and a 2,500-message flood were driven from a real frame and refused. Shadow DOM is a named limit: a click inside a web component captures the host with an empty snippet, which is correct and incomplete. (`src/preview/preview-message.js`, `src/preview/preview-picker-script.js`, `renderer/modules/preview-pane.js`, `docs/plans/2026-09-05-preview-select-security-findings.md`)
- **Desktop, tablet, phone: the pane can show the page at a device width.** A three-way control on the bar clamps the page to 640 or 380 CSS pixels, or lets it fill the pane. The clamp is a class on the container and never re-navigates the frame, so a width change keeps your scroll position and your picked outlines. The choice is remembered per app, like the split ratio. The resizer allocates screen between chat and page; this asserts a width about the page alone, which is why it belongs on the chat pane and not only in a later Builder. (`renderer/modules/preview-pane.js`, `renderer/styles/preview.css`, `docs/plans/2026-09-06-preview-step6-width-picker-handoff.md`)
- **The chat remembers every page it showed you.** A single reply routinely names six pages and the chip only ever offers the newest, so the chat header gains a page list beside the Timer, hidden until the chat has seen a page. It lists all of them, previewable or not; a row the root policy refuses opens the browser and says why. The list is keyed by the Claude session and kept in the app's profile, as paths only: a file on disk can never say a page is servable, because every restored path re-enters through the same policy check. The bar also names a script fault now, not only blocked external resources: an error thrown inside the sandboxed page is attributed to that pane and shown, so a half-dead page does not read as a network problem. (`src/preview/offered-store.js`, `src/preview/offered-pages.js`, `src/preview/preview-console.js`, `renderer/modules/preview-pages.js`)
- **Activity: what ACE is doing and has done, in one place.** A new view under Sessions. Three live sections partition every open run: *Blocked on you* for a run parked on a question or a permission wall, *Working* for the rest, *Finished* for results you have not cleared. Each row is named in ACE's own words when the reply ends with its `<!-- ace:did … -->` note, and by its receipts otherwise: "Edited 2 files in 02-Rituals", "Ran 3 commands", "Read 3 files", "sent an email". Receipts sit under the name as evidence, each with an `open` where a path exists. A failure carries `got it`; a success clears itself the moment the next turn starts. The row's name opens the conversation that produced it, when that conversation is still open. Beneath the live view, one folded line, *Earlier · N runs in the last 30 days*, unfolds the month by day from the journal. The view holds nothing the journal does not: prompts never reach the runtime, display labels are stripped before disk, and a run above normal sensitivity shows no name at all. (`renderer/views/activity.js`, `src/runtime-signal/`, `docs/plans/2026-09-02-three-lists-one-decision-ia-plan.md`, `docs/plans/2026-09-05-activity-earlier-fold-decision.md`, `docs/plans/2026-09-05-runs-in-aces-own-words-decision.md`)
- **The runtime signal underneath it, watching two of ACE's three engines.** A main-process store and journal record every chat turn and every background agent job as a run with a phase, needs, receipts and a terminal. Chat turns arrive through a tap on the chat manager; background jobs arrive through the daemon roster, so a job that survives an app restart keeps its run, a job the daemon lost is closed as stranded rather than shown as working for three days, and a job whose name arrives a beat after its run does gets the name. A turn blocked on a permission the operator has not granted parks as *Blocked on you* instead of finishing green, and clears itself when the approval sends the continuation. A connector call still out after ten seconds marks the run as waiting on something that is not ACE. A parked run restored from a checkpoint, which nothing could clear before, gets a `let it go` control on its row. Sovereign Mode is not yet a source; a Sovereign turn produces no run. The journal is a plain file under the app's data directory, capped at 30 MB, with a checkpoint written before every rotation and at quit. (`src/runtime-signal/store.js`, `src/runtime-signal/reducer.js`, `src/runtime-signal/chat-adapter.js`, `src/runtime-signal/agents-adapter.js`, `src/runtime-signal/journal.js`, `docs/plans/2026-09-03-runtime-vocabulary-widening-plan.md`, `docs/plans/2026-09-06-background-agent-adapter-plan.md`)
- **The Hearth orb carries runtime state, and a card on demand says what changed.** The seat orb at the bottom of the window now reads the same runtime signal: working, blocked, failed and rest each own every colour-bearing layer, so the states are told apart at a glance rather than by tint. Tender outranks every register: when the nervous-system read asks for care, the orb softens amplitude and tempo and only a failure keeps its dim red ring. A quiet bar line beneath the orb names the one thing worth knowing, and the card opens on demand with the subject in ACE's words, its receipts and its route. Ambient keeps its one next move; the runtime contributes at most one candidate to that decision and never a list. (`renderer/modules/guide-hearth.js`, `renderer/modules/hearth-card.js`, `renderer/views/ambient.js`, `renderer/styles/views/ambient.css`, `docs/plans/2026-09-02-hearth-orb-visual-language-spec.md`)
- **A scheduled task that stopped firing says so.** A task suppressed by failure backoff used to record everything correctly in a sidecar nobody opens; one ran silent for eighteen days. On the first roster poll after launch, and every thirty minutes on platforms without launchd, ACE reads the sidecars and announces a task that has crossed the suppression threshold, re-announces a deepening outage at intervals rather than nagging, and forgets a recovered task so its next outage announces fresh. The roll-up reads the platform-agnostic health sidecar when there is no launchd log, so a task running fine on Windows or Linux stops being reported as missed. (`src/scheduler/health-digest.js`, `src/scheduler.js`, `tools/scheduler-rollup` at the vault root)
- **A tool card says what ACE is doing, not how many things it did.** "6 operations" becomes "Editing session-1-synthesis.md · step 3", "Searched Gmail", "Checked whether the Spanish div is hidden". Every label is a projection of the input, never the input: file names not paths, host names not URLs, the description not the shell arguments, with anything token-shaped suppressed. Checked against 11,259 real tool groups from 433 past sessions with zero leaks. Finished cards demote to a quiet line so a long turn reads as a column of quiet lines and one live card. (`renderer/modules/op-phrases.js`, `renderer/modules/tool-renderer.js`, `scripts/ops-label-corpus-gate.mjs`)
- **Speak, then press send. You no longer have to stop the mic first.** The arrow closes the recording, waits for the words, and sends them, in chat, in the Oracle panel and in the capture bar. It shows a spinner across the transcription wait and no longer paints itself as a stop square mid-dictation. A recording that comes back silent sends nothing and cancels nothing. (`renderer/modules/voice-input.js`, `renderer/modules/chat-pane.js`, `renderer/views/oracle.js`)
- **Listen to a reply.** A settled assistant reply gains a Listen button: Deepgram Aura when a key is configured, the system voice otherwise, with a failed Deepgram attempt falling back to the system voice rather than going silent. (`renderer/modules/chat-tts.js`)
- **Connectors say whether they worked, not only whether they are configured.** Each card now carries two independent lines: set up in ACE (from config) and worked in the last session (from what the session actually announced it loaded). One config reader replaces two that each opened a different, incomplete set of scopes; the fourth scope, the project entry in `~/.claude.json`, is where `claude mcp add` writes by default and where a working connector had sat unrecognised for weeks. (`src/connectors/mcp-config.js`, `renderer/views/connectors.js`)
- **Fable 5.1 in the model picker.** Replaces Fable 5; `fable-5` and the bare `fable` alias remap to 5.1. Needs CLI 2.1.251 or later. (`src/model-catalog.js`)

### Fixed
- **On-device Whisper, four fixes found by a member on the shipped Windows build.** The bundled `whisper-cli` was compiled without AVX2/BMI2/FMA on hardware that supports them; the thread count could fall to one and starve a small machine, and is now clamped between four and eight; a model downloaded from Settings landed outside the path `/transcribe` searched; and the large-v3-turbo tier is offered with a note about what it costs to run. The AVX2 flag cannot be verified on this Mac; the banner reading `AVX2 = 1` only becomes observable on a CI-built x64 binary. (`scripts/build-whisper.sh`, `src/connectors/whisper-run.js`, `src/model-catalog.js`)
- **The system prompt no longer tells ACE it cannot run terminal commands.** It could, and the line was why pasting a link and asking for a transcript was declined on 0.4.6. (`src/chat-manager.js`)
- **Two scheduled tasks firing at the same minute no longer silently kill one of them.** Dispatches are spaced 25 seconds apart through an on-disk gate that fails open; a run that dies before init is retried once; the failure card states the next fire time as fact. (`src/scheduler/scheduler-exec.js`, `src/agents-retry-policy.js`, `renderer/modules/agent-failure-copy.js`)
- **A scheduled fire can no longer trip the sync that kills it.** Every dispatch created a gate directory under the tasks root, the file watcher reported it, and 300 ms later the app rewrote every launchd plist and booted each job out and back in, killing the fire between spawning its job and writing its first log line. Caught live with the process still warm. The watcher now reloads only on a task's own `SKILL.md` or its directory appearing or disappearing; everything else under the root is scheduler state and never reaches reload. (`src/scheduler/watch-filter.js`, `src/scheduler.js`)
- **A background job the daemon lost is no longer shown as running.** One sat under RUNNING for three days after dying four seconds into its launch, with twelve more behind it going back to June. A job the daemon does not know, whose state file has been quiet for fifteen minutes, now reads as stopped in the Agents view and closes as stranded in Activity. Reported, never written: the job directory stays for the operator to remove. (`src/agents-stranded.js`, `src/agents-manager.js`, `src/runtime-signal/agents-adapter.js`)
- **A reloaded chat no longer loses its resume id when its transcript is not where the app first looks.** On a vault reached through a symlink, the renderer derived one project folder name and the CLI wrote under another, so every history read came back "not found" and a recent change treated that as proof the chat was dead, wiping its id while the transcript sat intact on disk. The reader now searches sibling project folders by session id before reporting a miss, and a failed read never nulls a chat's ids; the next send decides, and the stored folder self-heals once a real read succeeds. (`src/session-reader.js`, `renderer/modules/session-manager.js`, `renderer/modules/history-restore.js`)
- **A dropped connection no longer eats a dictation.** Deepgram transcription treated one flaky moment as terminal, with no retry and the audio discarded. Now three attempts with backoff, the recording kept until the last one fails, and a network failure named in the toast rather than surfacing as a raw `fetch failed`. (`src/insight-transcribe.js`)
- **The turn note no longer leaks into the reply.** The strip that removes ACE's trailing `<!-- ace:did … -->` record also removed an unclosed `<!--` from the growing buffer, which deleted the opener the moment it streamed in and left the rest of the note in the reply as plain text. A closed note is stripped from the accumulator; the anti-flash strip of a half-written marker moved to the three render sites, where being wrong costs one frame rather than the reply. (`renderer/modules/session-manager.js`, `src/runtime-signal/chat-adapter.js`, `renderer/modules/chat-renderer.js`)
- **The bottom bar's date rolls over at midnight.** It was written once at mount; an app left running showed its launch day for a week. Rescheduled from calendar fields so DST cannot land it an hour off, and re-rendered on wake because Chromium timers do not advance while the machine sleeps. (`src/guide/guide-core.js`, `renderer/modules/guide-hearth.js`)
- **A disposed preview frame no longer takes the main process down.** The navigation guard read `event.frame` behind a comment saying it may be null; it is a getter that throws once the frame is gone, and it escaped as an uncaught exception. Fails closed now: no current URL means the frame cannot prove its entitlement and the target-only rule decides. (`src/preview/preview-navigation.js`)
- **Preview text is served as UTF-8**, so an em dash in a page with no `<meta charset>` renders as an em dash and not as mojibake. (`src/preview/preview-protocol.js`)
- **The dashboard was under-counting overdue follow-ups.** Overdue detection required the Due cell to begin with a bare ISO date; the vault writes them bolded, trailed by the date they replaced, or as `Week of` and month-only values. Three overdue rows showed where seven existed. Parsing now lives in one tested module, reads the first date in the cell, resolves a month-only value to its last day, and counts days on the local calendar. (`renderer/modules/followup-due.js`, `renderer/dashboard.js`)
- **What ACE says between tool rounds no longer vanishes.** Narration is sealed the moment a tool starts, so the card lands after the sentence it follows. A text block the model opens and never fills no longer splits one piece of work into two cards. (`renderer/modules/session-manager.js`, `renderer/modules/tool-renderer.js`)
- **Captures are stamped in local time, and a failed capture write is reported as a failure.** (`src/capture.js`)
- **Runtime run ids are unique across app restarts**, so the second launch's first turn no longer collides with a run from the previous launch and goes unreported. (`src/runtime-signal/chat-adapter.js`)

### Changed
- **`got it` is offered on failures only.** Starting the next chat turn already acknowledges every completion it displaces, so the button on a success was only ever visible because the operator stopped to look. (`renderer/views/activity.js`, `renderer/modules/hearth-card.js`)
- **Runtime retention is a policy rather than an accident.** Unresolved state never expires; cleared history keeps 30 days; the byte ceiling reports when it fires rather than silently shortening the app's memory. (`src/runtime-signal/journal.js`, `docs/plans/2026-09-02-runtime-retention-and-labelling-decision.md`)
- **The Fathom connector card describes a REST key, not an MCP server.** `/process-call` reads the sweep cache and falls through to the REST API; the MCP server was unreachable on fourteen of fourteen unattended probes. (`src/connector-registry.js`)

### Known limitations
- **Windows and Linux are unverified on this candidate.** No build was produced for either and nothing was run. The scheduler failure-visibility work carries tests for both, and the Windows history-folder defect (`windows-history` on the roadmap) is still open.
- **Voice transcription on-device still depends on the build that produced the DMG.** This local arm64 candidate bundles `ffmpeg` and the base model, as 0.4.6 did on this machine, and no `whisper-cli`; the CI build is what bundles the engine for members.
- **Unsigned.** Members installing this will see the Gatekeeper warning; the signing pipeline is built and waits on the Apple Developer enrolment.
- **The freeze/reply-loss cluster remains open.** Nothing in this release should be read as a fix for it. Carried unchanged from 0.4.6.
- **Sovereign Mode turns do not appear in Activity or the runtime signal.** Only chat turns through the Claude path and background agent jobs are observed, and on this candidate a job dispatched headless by the scheduler completed without ever appearing as a run (see the release record). The Agents view still shows it.
- **The preview pane is session-scoped.** Cmd+R drops every pane and revokes every capability; the chip returns on the next sighting.
- **Activity rows from previous sessions are mostly not clickable.** The runtime's chat id and History's session id are two identifier spaces, and the mapping is its own piece of work.
- **If you get signed out of Claude, the app will not tell you, and cannot sign you back in.** Carried from 0.4.4 onward; recovery is `claude auth login` in a terminal.

---

## [v0.4.6](https://github.com/actualize-ace/ace-desktop/releases/tag/ace-desktop-v0.4.6) — 2026-08-20 — your first session starts trusted, and the bolt shows what the inbox is carrying

Two things sat behind this release. The first is a defect 0.4.5 shipped with: on a brand new install, the very first session ran with all of its permissions silently thrown away, and scheduled background tasks ran that way every single time. The second is that the inbox could fill to the point of being unusable without anything ever saying so. Both are closed here.

**This release has a companion vault update.** The rituals that count your inbox were rewritten at the same time, and they live in your vault rather than in the app, so installing this build is only half of it. Run `/sync-core` after upgrading to get the matching side.

### Added
- **The titlebar bolt now shows what the inbox is carrying.** The element you press to capture also reads as a gauge: it warms from its normal state through yellow to amber as the inbox fills, with a count badge appearing once it is worth acting on. It reads two things, how many open items are sitting there and how much text they add up to, and reports whichever is worse. Silence stays the default, so an inbox in good shape looks exactly as it always has. The ramp deliberately stops short of alarm red, because a full inbox is asking for twenty minutes of routing, not an emergency. Pressing the bolt always opens capture, in every state, so the thing you reach for most never moves; when the inbox is heavy, a quiet line inside the capture box offers to triage instead. State loads at startup rather than waiting for a file to change, so an already-heavy inbox reads correctly the moment you open the app instead of looking calm until you touch something. (`src/intelligence/inbox-load.js`, `src/file-watcher.js`, `renderer/styles/shell.css`)
- **Voice notes transcribe with nothing to install.** ACE now bundles its own `whisper.cpp` engine and a self-built LGPL `ffmpeg`, so a voice note converts to text on-device, on macOS (Apple Silicon and Intel), Windows, and Linux, with no account, no key, and no upload. Settings gains a model manager to download a more accurate tier (`ggml-large-v3-turbo`) with live progress and one-click removal. The in-chat mic can now use this same on-device engine directly, as a third Voice Provider option alongside Deepgram and a self-hosted Whisper server, so speaking into ACE no longer requires a paid key. If the engine can't be found, ACE says exactly what's missing rather than staying silent. (`scripts/build-whisper.sh`, `scripts/build-ffmpeg.sh`, `src/model-catalog.js`, `src/model-manager.js`, `src/connectors/whisper-run.js`, `src/insight-transcribe.js`, `renderer/modules/voice-providers.js`, `resources/licenses/THIRD-PARTY.md`)

### Fixed
- **Your first session no longer runs with its permissions silently dropped, and neither do scheduled tasks.** On a fresh install, the very first session printed `Ignoring 15 permissions.allow entries from .claude/settings.json: this workspace has not been trusted` and ran without any of them. It repaired itself invisibly on the second launch, so the damage was confined to the first session, which is exactly the session a new member judges the whole system by. The cause was a wiring gap rather than a missing feature: the code that marks a vault as trusted has existed since 0.4.1 and its unit tests all passed, but on a fresh install nothing called it at the moment the vault first became known. Scheduled background tasks were worse and had no coverage at all, because they launch without going through the app at all, and a background run has nobody present to answer a trust prompt. Trust is now stamped on both paths, plus a third that a future vault-switch would have gone through. Verified against the live CLI rather than assumed, including confirming that a concurrent session does not overwrite the stamp. (`main.js`, `src/vault-guards.js`, `src/save-config-core.js`, `src/scheduler/dispatch-core.js`)
- **The Cadence Ring stops reporting green across a gap it never reviewed.** The ring read `M: 1d` in the middle of a 46-day gap. When a monthly review carried no completion stamp, the reader fell back to the file's creation date on disk, and that date is reset by an ordinary sync or checkout, so the clock silently reset itself to "just done" every time the vault moved. Reviews are now identified by their filename the way the original design specified, a skipped period is recognised as a skip rather than counted as a completion, and the creation-date fallback is gone entirely. A missing or skipped period now ages the ring from the last real review. (`src/vault-reader.js`)

### Changed
- **A cancelled turn now records why it was cancelled, and both halves of a turn share an ID.** Diagnostic groundwork for the blank-bubble investigation, with no change to what you see. Every place a turn can be stopped now labels itself, a teardown is only recorded as a kill if it actually landed on a live process, so a turn that finished a moment before you pressed Stop no longer reads as killed after the fact, and the app-side and interface-side accounts of the same turn now join on a shared identifier instead of being matched up by timestamp by hand. That hand-matching is what made the August 10 silent-turn diagnosis so slow. (`src/chat-telemetry.js`, `src/chat-manager.js`)

### Security
- **Hardened several IPC handlers against unvalidated input**, and fixed a spot where attachment data was rendered into the page without escaping.
- **Closed path-traversal issues in the vault-scaffolding and artifacts layers**, and tightened validation on connector test requests and MCP server definitions.
- **Fixed a cross-project information leak in MCP server listings**, an extension-bypass gap in attachment handling, a path-containment/denial-of-service gap in connector health checks, and a Windows command-injection risk in the audio synthesizer.
- **Updated Electron to 39.8.10** (from 34.5.8) to pick up upstream security fixes; electron-builder was updated alongside it.
- **Removed an unused dependency carrying known vulnerabilities**, and scrubbed a hardcoded operator path out of two publishable skills.
- **Internal engineering files no longer ship inside the installed app.** Audit reports, internal docs, and build tooling were previously bundled into every DMG, EXE, and AppImage; the packaging allowlist now excludes them.

Closes all 15 actionable findings from an internal security audit (2 critical, 9 high, 4 medium). Full vitest suite green throughout (2055+ tests, no regressions), with every IPC-facing fix additionally hand-verified live against the running app.

### Known limitations (carried from 0.4.5, still true)
- **The blank-bubble failure remains visible, not fixed.** The instrumentation stands and this release widens it, but no occurrence has yet named which of the two surviving mechanisms is real. Nothing here should be read as a fix for it.
- **If you get signed out of Claude, the app will not tell you, and cannot sign you back in.** Carried unchanged from 0.4.4 and 0.4.5: the signed-out card and chip do not fire on a real sign-out, and recovery is `claude auth login` in a terminal. The fix remains carded as W2.1a.
...
**Correction 2026-08-20: the sentence below claiming "ACE holds a read-scoped token, so 'ACE cannot send' is enforced by the token, not by a promise" was not true of this build.** ACE could not send, and that part stands: there is no send route in the write client and a guard test holds it there. But the *credential* installed at 0.4.5 carried Beeper's `write` scope, so send was prevented by the application, not withheld by the token. The token was rotated to read-only on 2026-08-20.

The framing was also wrong in a way rotating a token does not fix, and it is worth stating plainly because it applies to every connector after this one. Token scope is chosen by whoever mints it — Beeper's "Allow sensitive actions" toggle — and ACE receives whatever was pasted. So "ACE holds a read-scoped token" was never a property of the build; it was a property of one install, and it varied per member. The two guarantees are separate and only one of them is ours to make: **ACE never sends, for every member, because there is no send route in the client** — and, independently, *your* credential can be narrowed to read-only, which is worth doing as defence in depth. A future build will introspect the credential at startup and tell you which one you are holding.
...
The v0.4 keel lands stable. The Agents and Images previews from v0.3.8–v0.3.9 are now first-class, background scheduling runs on macOS launchd, an Ambient surface and an opt-in Sovereign Mode arrive, Intelligence briefings render in-app, and the freeze / lost-work / stall bugs are fixed. This entry graduates the rc.1 → rc.2 scope below and folds in everything cut since rc.2.
...
## [v0.4.0-rc.2](https://github.com/actualize-ace/ace-desktop/releases/tag/ace-desktop-v0.4.0-rc.2) — 2026-06-25 — RC refresh (People view · chat resilience · agent-dispatch + stall fixes)
