...
### Added
- **Speak, then press send. You no longer have to stop the mic first.** Dictating took two presses to finish: one on the microphone to stop and transcribe, another on the arrow to send. Pressing the arrow mid-dictation did not do the obvious thing, it did something worse. The text box is disabled and empty for the whole recording, so the arrow read an empty box: in chat that means "cancel", so the press could kill the reply ACE was streaming; in the quick capture bar it closed the overlay and dropped the recording without transcribing, so the thought vanished with no toast to say it had. The arrow now closes the recording, waits for the words, and sends them, in chat, in the Oracle panel, and in the capture bar. The microphone button is unchanged, so the review step is still there whenever you want to fix a misheard word before it goes. Two smaller repairs came with it. The arrow shows a spinner and goes unclickable across the one-to-two-second transcription wait, because a button that does not move for a second gets pressed twice. And it no longer paints itself as a stop square while you are dictating, which it did whenever a reply was streaming, since the box is always empty mid-recording: the button was saying "stop" while the click would have sent. A recording that comes back silent or fails now sends nothing and cancels nothing. (`renderer/modules/voice-input.js`, `renderer/modules/chat-pane.js`, `renderer/views/oracle.js`, `renderer/index.html`, `renderer/styles/chat.css`, `renderer/styles/overlays.css`, `renderer/styles/shell.css`)

- **The tool card says what ACE is doing, not how many things it did.** A card that read "6 operations" told you the renderer counted six things. It now reads "Editing session-1-synthesis.md · step 3" while the work moves, "Edited session-1-synthesis.md · 6 steps" once it settles, "Checked whether the Spanish div is hidden" for a shell step that ACE described, "Searched Gmail" or "Send via Gmail" for a connector call. The wording comes from two small literal tables, never from a grammar guesser, and settled labels only claim what the turn can vouch for: a cancelled turn's open card says "Stopped after 4 steps", an errored one "Ended with an error after 4 steps", and nothing ever settles to "sent", "published", "deleted" or "deployed". Every label is a projection of the input, never the input: file names not paths, host names not URLs, the description not the shell arguments, the connector and action not the Gmail query, with anything that looks like a token, an address, or an identifier suppressed. Checked, not assumed: every label the card could show was rendered for 11,259 real tool groups from 433 past sessions and scanned, with zero hits. Rows lost their "Bash:" prefixes and truncated commands for the same phrasing plus a small tool chip, an edit row shows the span it replaced ("85→210 chars", "4→11 lines"), a written file shows its real size in bytes, and the expanded detail is an indented hairline instead of a third nested box. Only the card that is live keeps its frame and a slow accent sweep through the text; a finished card demotes to a quiet line, so a long turn reads as a column of quiet lines and one live card. The screen-reader name is the constant "Operations", with expansion, busy state, and the step count carried by attributes and a hidden status, and Enter or Space toggles the card from the keyboard. (`renderer/modules/op-phrases.js`, `renderer/modules/tool-renderer.js`, `renderer/styles/chat.css`, `scripts/ops-label-corpus-gate.mjs`)

### Fixed

- **The dashboard was under-counting overdue follow-ups, which is the one direction a health signal must never fail in.** Overdue detection required the Due cell to *begin* with a bare `2026-08-18`. The vault does not write them that way: the common form is bolded (`**2026-08-28**`), often trailed by the date it replaced (`(was 2026-08-25, missed)`), alongside `After 2026-07-07`, `Week of 2026-08-17` and month-only values like `July 2026`. Every one of those failed the check and was dropped without a trace, so the list rendered clean while commitments aged behind it. Measured against a real vault on 2026-09-02, three overdue rows showed where seven existed, and a 20,000 MXN commitment written down as due `July 2026` had gone 33 days unsurfaced. Parsing now lives in one tested module and is permissive about form while staying strict about evidence: a date is read only when a real one is present, so `TBD`, `Session 2`, `v0.4.0` and `Divine timing` still resolve to nothing rather than being coerced into a deadline, and an impossible date like `2026-02-30` is rejected instead of rolling into March. Two rules that are easy to get backwards are now pinned by tests. The first date in the cell wins, not the last, because the convention writes the live date first and its history after. And a month-only value resolves to the last day of that month, so `July 2026` is not reported late on July 2nd. A third was found while writing it: the day count reads your local calendar rather than UTC, which otherwise pushed every row a day later each evening west of Greenwich. The suite runs under three timezones so that cannot come back. (`renderer/modules/followup-due.js`, `renderer/dashboard.js`, `docs/plans/2026-09-02-followups-first-class-view.md`)
- **What ACE says between tool rounds no longer vanishes.** A long turn looked like a wall of identical "6 operations" cards with nothing between them. The transcript behind that screen had a sentence before every one of those rounds ("Spec and evidence read. Now pulling the six source files…"), and each one was wiped the moment the next sentence began. The step that was meant to keep the previous paragraph looked it up with a selector that can never match, skipped the keep, and then emptied the box the paragraph was still sitting in. Narration is now kept the moment a tool starts, so the card lands after the sentence it follows and the next sentence renders after the card. Two related repairs: a text block the model opens but never fills, which the API does between tool rounds, no longer splits one piece of work into two cards, and a turn's final sentence before a last tool round is no longer painted over with nothing when the turn settles. (`renderer/modules/session-manager.js`, `renderer/modules/tool-renderer.js`)
- **Two agents scheduled at the same minute no longer silently kill one of them.** The morning failures ("exit 1 before init", an empty reasoning trail, no explanation) turned out to be deterministic, not flaky: the Claude daemon keeps exactly one pre-warmed spare worker, and its cold fresh-spawn path has crashed on every single occurrence in the daemon log's history — 16 of 16, across months of versions. So when two tasks fired in the same second (gmail-morning-triage and intel-run at 8:00), whichever lost the race for the one spare always died, alternating victims day to day; a cold daemon start killed even a lone task the same way. Three defenses now stand, and none of them require the user to know any of this. Scheduled dispatches are spaced 25 seconds apart across all tasks via an on-disk gate, so the daemon's replenishment spare has time to boot between fires; the gate fails open, so spacing can never cost a fire. A run that dies before init is auto-retried once after a short delay — including senders, because a worker that never initialised provably sent nothing, so the double-send guardrail does not apply to this failure class. And the failure card now states the next scheduled fire time as fact, so a failed run reads as "recovering, next run tomorrow at 8:03 AM" instead of a dead end that leaves you wondering whether the agent will ever run again. (`src/scheduler/scheduler-exec.js`, `src/agents-retry-policy.js`, `src/agents-manager.js`, `renderer/modules/agent-failure-copy.js`, `renderer/modules/agents-manager.js`)
